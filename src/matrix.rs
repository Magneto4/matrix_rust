...
impl<T: Trait, const R: usize, const C: usize> MulAdd<T, Matrix<T, R, C>> for Matrix<T, R, C> {
  type Output = Matrix<T, R, C>;

  fn mul_add(self, a: T, b: Matrix<T, R, C>) -> Matrix<T, R, C> {
      let mut res = [[T::default(); C]; R];
      for i in 0..R {
          for j in 0..C {
              res[i][j] = self.values[i][j] + a * b.values[i][j];
          }
      }
      Matrix::from(res)
  }
